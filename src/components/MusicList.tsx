  ...
  return (
    ...
    {view === 'table' ? (
      <table className="table-auto w-full text-leftw-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          {files.map((file) => (
            <tr key={file.id} onClick={() => onPlay(file.id)} className={`bg-white border-b dark:bg-gray-800 dark:border-gray-700 ${file.id === currentId ? 'bg-green-100' : ''}`}>
                <td className="pr-6 py-4">{file.id+1}.</td>
                 <td className="py-4">
                  {file.AudioData.image ? (
                    <Image  src={file.AudioData.image} alt={file.name} width={100} height={100} className="w-16 h-16" />
                  ) : (
      ...
      />
                ...
                <td className="px-6 py-4">
                    <div>
                    {removeExtension(file?.AudioData?.name)}
                    </div>
                    <div>
                    {file.AudioData.artist || 'Unknown'}
                    </div>
    ...
    {!files.length && (
        <h2>Please Add Audio Items To Use Music Player</h2>
    )}
  </div>
  )
}
